	    ...
	    case 'checkbox':
			...
			for (var x = 0; x < elements.length; x++) {
				...
				if (order == 1)
					...
					parameter += "&" + element.name + "=" + $F(element);
			}
			retString = sourceString.replace(regExp, parameter);
			break;
...
AjaxJspTag.Select.prototype = (new AjaxJspTag.Base()).extend({
  ...
  initialize: function(url, options) {
    ...
    this.options.targetElem.onchange();
  },
  ...
  populateSelect: function(xml) {
    ...
    var currentValue = respNode.getElementsByTagName("currentValue")[0].firstChild.nodeValue;
    for (var i=0; i<items.length; i++) {
      if (items[i].getElementsByTagName("name")[0].firstChild==null) {
        ...
        this.options.targetElem.options[i] = option;
      }
    }
  },
...
AjaxJspTag.Autocomplete.prototype = (new AjaxJspTag.Base()).extend({
  ...
  setPopupStyles: function() {
    ...
    if (isIE) {
      $(this.popupElem).style.width = (this.options.sourceElem.offsetWidth+150) + "px";
    } else {
